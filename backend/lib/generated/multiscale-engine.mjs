...
function getEquipmentToken(equipment) {
    return `${equipment.id} ${equipment.name} ${equipment.type}`.toLowerCase();
}
function inferContextMaterialIds(experiment, contextText) {
    const materialIds = [];
...
function inferMaterialIdsFromEquipment(equipment, experiment, contextText) {
    const token = getEquipmentToken(equipment);
    const materialIds = [];
    if (includesAny(token, ['wire', '导线', 'connector', 'circuit-line']))
        materialIds.push('copper', 'plastic');
    if (includesAny(token, ['battery', '电池', 'power']))
        materialIds.push('zinc', 'graphite', 'electrolyte', 'plastic');
    if (includesAny(token, ['bulb', '灯泡', 'light']))
        materialIds.push('glass', 'tungsten', 'copper');
    if (includesAny(token, ['switch', '开关']))
        materialIds.push('copper', 'plastic');
    if (includesAny(token, ['resistor', '变阻器', 'rheostat', 'slider']))
        materialIds.push('graphite', 'copper', 'plastic');
    if (includesAny(token, ['ammeter', 'voltmeter', '电流表', '电压表', 'meter']))
        materialIds.push('copper', 'glass', 'plastic');
    if (includesAny(token, ['electrode', '电极']))
        materialIds.push('metal-electrode', 'electrolyte');
    if (includesAny(token, ['salt-bridge', '盐桥']))
        materialIds.push('electrolyte', 'water', 'cellulose');
    if (includesAny(token, ['dropper', '滴管', 'burette', 'pipette']))
        materialIds.push('rubber', 'plastic', 'glass');
    if (includesAny(token, ['paper', '试纸']))
        materialIds.push('cellulose', 'indicator-dye');
    if (includesAny(token, ['indicator', '指示剂']))
        materialIds.push('indicator-dye', 'water');
    if (includesAny(token, ['solution', '溶液']))
        materialIds.push('water');
    if (includesAny(token, ['starch', '淀粉']))
        materialIds.push('starch', 'water');
    if (includesAny(token, ['iodine', '碘']))
        materialIds.push('iodine-reagent', 'water');
    ...
    if (materialIds.length === 0) {
        if (experiment.subject === '物理')
            materialIds.push('steel', 'plastic');
        if (experiment.subject === '化学')
            materialIds.push('glass', 'water');
        if (experiment.subject === '生物')
            materialIds.push('cell-membrane', 'cytoplasm', 'water');
        if (experiment.subject === '科学')
            materialIds.push('plastic', 'water');
    }
...
function inferComponents(equipment, materialIds, contextText) {
    const token = getEquipmentToken(equipment);
    const mainMaterial = materialIds[0];
    const secondaryMaterial = materialIds[1] ?? mainMaterial;
    const findAltMaterial = (fallback) => materialIds.find((materialId) => materialId !== mainMaterial) ?? fallback;
    if (includesAny(token, ['battery', '电池', 'power'])) {
        return [
...
function inferEquipmentProfiles(experiment) {
    ...
    const equipmentProfiles = experiment.equipment.map((equipment) => {
...
function deriveReactionRules(experiment, materials, contextText) {
...
function deriveDefaultLens(experiment, contextText, materialIds) {
    if (includesAny(contextText, ['电路', '电流', '电压', '欧姆', '磁', 'magnet', 'wire', 'battery', 'resistor', 'rheostat', '透镜', '棱镜', '折射', 'reflection', 'refraction'])
...
function findFocusEquipment(experiment, step, focusTargetObject) {
    const focusText = `${focusTargetObject ?? ''} ${step?.title ?? ''} ${step?.description ?? ''}`.toLowerCase();
    if (!focusText.trim())
        return experiment.equipment[0] ?? null;
    return (experiment.equipment.find((equipment) => {
        const token = `${equipment.id} ${equipment.name} ${equipment.type}`.toLowerCase();
        return (focusText.includes(equipment.id.toLowerCase())
...
function deriveFocusedLens(step, fallback, materialCount, speciesCount, hasRule) {
    if (!step)
        return fallback;
    if (speciesCount > 0
...
export function getExperimentMultiscaleView(experiment) {
...
export function getFocusedExperimentMultiscaleView(experiment, options = {}) {
    ...
    const focusProfile = multiscale.equipmentProfiles.find((profile) => profile.equipmentId === focusEquipment?.id)
        ?? multiscale.equipmentProfiles[0]
        ?? null;
    const focusMaterialIds = Array.from(new Set((focusProfile?.components ?? []).map((component) => component.materialRef).filter((materialRef) => Boolean(materialRef))));
    ...
    const componentSummary = (focusProfile?.components ?? []).slice(0, 2).map((component) => component.name).join(' / ')
